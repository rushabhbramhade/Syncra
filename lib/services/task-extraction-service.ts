...
export async function extractFromText(userId: string, text: string, sourcePlatform?: string, sourceItemId?: string): Promise<ExtractedTask[]> {
  try {
...
export async function saveTasks(userId: string, tasks: ExtractedTask[]): Promise<void> {
...
export async function updateTaskStatus(userId: string, taskId: string, status: string): Promise<void> {
  const db = createAdminDb();
